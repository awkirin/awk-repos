...
if (-not (Test-Path -LiteralPath "C:\Windows\System32\VBoxService.exe")) { throw "VirtualBox Guest Additions are missing" }
if ((Get-Service VBoxService).Status -ne "Running") { throw "VBoxService is not running after restart" }
if (-not (Test-Path -LiteralPath "C:\Windows\Panther\SysprepUnattend.xml")) { throw "Sysprep answer file is missing" }
if (-not (Get-ScheduledTask -TaskName "PackerSysprep" -ErrorAction SilentlyContinue)) { throw "Sysprep task is missing" }
